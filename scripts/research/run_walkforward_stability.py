...
def read_latest_comparison(
    ...
    if out.empty:
        raise ValueError(
            f"No rows found in comparison output for feature_set={feature_set}."
        )
...
def add_period_baseline_deltas(df: pd.DataFrame) -> pd.DataFrame:
    ...
    key_cols = ["Period_Label", "Experiment_Mode"]
...
def build_reoptimized_summary(df: pd.DataFrame) -> pd.DataFrame:
    reopt = df[df["Experiment_Mode"].astype(str).eq("reoptimized")].copy()
    ...
    keep_cols = [col for col in keep_cols if col in reopt.columns]

    reopt = reopt[keep_cols].copy()
    reopt = reopt.sort_values(["Period_Label", "Score"], ascending=[True, False])
...
def build_stability_score(df: pd.DataFrame, min_edge: float) -> pd.DataFrame:
    reopt = df[
        df["Experiment_Mode"].astype(str).eq("reoptimized")
        & ~df["Feature_Set"].eq("baseline")
    ].copy()

    if reopt.empty:
        return pd.DataFrame()

    grouped = reopt.groupby("Feature_Set", dropna=False)

    summary = grouped.agg(
        Periods=("Period_Label", "nunique"),
        Pass_Periods=("Pass_Tradeable_Rule", lambda x: int(pd.Series(x).astype(str).str.lower().isin(["true", "1"]).sum())),
    ...
    summary["Stable_Strict_Candidate"] = (
    ...
    summary = summary.sort_values(
        ["Stable_Strict_Candidate", "Pass_Rate", "Avg_Edge", "Avg_Score"],
...
def write_markdown_summary(
    ...
    lines = []
    lines.append("# Walk-forward Stability Summary")
    lines.append("")

    if stability.empty:
        lines.append("No stability rows were generated.")
    else:
    ...
    if not reoptimized.empty:
        lines.append("## Reoptimized Period Results")
        lines.append("")
        lines.append(reoptimized.to_string(index=False))
        lines.append("")

    path.write_text("\n".join(lines), encoding="utf-8")


def run_walkforward(args: argparse.Namespace) -> None:
    ...
    backup_dir = project_root / ".tmp_walkforward_backups" / f"{ticker}_{profile}_{timestamp}"
    ...
    if not feature_sets:
        feature_sets = DEFAULT_FEATURE_SETS.copy()

    # Baseline is required for period-level deltas.
    ...
    try:
        for period_label, period_start, period_end in periods:
            ...
            if len(stock_period) < args.min_rows:
                print(
                    f"Skipping period={period_label}. Not enough stock rows: {len(stock_period)} < {args.min_rows}"
                )
                continue

            if len(index_period) < args.min_rows:
                print(
                    f"Skipping period={period_label}. Not enough index rows: {len(index_period)} < {args.min_rows}"
                )
                continue
    ...
    if not all_rows:
        raise RuntimeError("No walk-forward rows were collected.")

    combined = pd.concat(all_rows, ignore_index=True)
    combined = add_period_baseline_deltas(combined)

    reoptimized = build_reoptimized_summary(combined)
    stability = build_stability_score(combined, min_edge=args.min_edge_vs_breakeven)

    raw_path = output_dir / f"{ticker}_{profile}_walkforward_raw.csv"
    reopt_path = output_dir / f"{ticker}_{profile}_walkforward_reoptimized_summary.csv"
    stability_path = output_dir / f"{ticker}_{profile}_walkforward_stability_score.csv"
    markdown_path = output_dir / f"{ticker}_{profile}_walkforward_summary.md"

    combined.to_csv(raw_path, index=False, encoding="utf-8-sig")
    reoptimized.to_csv(reopt_path, index=False, encoding="utf-8-sig")
    stability.to_csv(stability_path, index=False, encoding="utf-8-sig")
    write_markdown_summary(markdown_path, stability, reoptimized)
    ...
    if stability.empty:
        ...
        display_cols = [
            ...
            "Avg_Return",
            "Total_Trades",
            "Stable_Strict_Candidate",
        ]

        display_cols = [col for col in display_cols if col in stability.columns]
        print(stability[display_cols].to_string(index=False))


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run period stability tests by temporarily filtering local data files."
    )

    parser.add_argument("--ticker", type=str, required=True)
    parser.add_argument("--profile", type=str, default="SWING")
    parser.add_argument("--mode", type=str, default="both", choices=["fixed", "reoptimized", "both"])
